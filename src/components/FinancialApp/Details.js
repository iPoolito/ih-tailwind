...
export default function Details() {
  ...
  useEffect(() => {
    const getResponse = async () => {
      ...
      const res = await axios.get(
        `https://api.exchangerate.host/timeseries?start_date=${date.startDate}&end_date=${date.endDate}&base=USD&symbols=${currency}`
      )
      ...
      setDataChart({
        ...
        datasets: [
          {
            label: `¿Cuánto vale un dólar en ${currency}?`,
  ...
  return (
    <div>
      ...
      {loading ? <p>Cargando...</p> : <Line data={dataChart} />}
    </div>
  )
}
